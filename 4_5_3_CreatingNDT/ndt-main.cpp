...
double Probability(Eigen::MatrixXd X, Eigen::MatrixXd Q, Eigen::MatrixXd S){
	...
	// The (0,0) in the following equation is the indexing of a 1x1 Eigen::MatrixXd, so 
	// we can forward it to math.h functions
...
void NewtonsMethod(PointT point, double theta, Cell cell, Eigen::MatrixBase<Derived>& g_previous, Eigen:: MatrixBase<Derived>& H_previous){
	...
	q_p3(1,0) =  X(0,0)*cos(theta) - X(1,0)*sin(theta);
	// TODO: calcualte the 1 x 1 exponential matrix which uses q, and S inverse
	Eigen::MatrixXd exp_q(1,1);
	exp_q(0,0) = exp(-0.5*(q.transpose()*Si*q)(0,0));
	...
	g(0,0) = (q.transpose()*Si*q_p1*exp_q)(0,0);
	g(1,0) = (q.transpose()*Si*q_p2*exp_q)(0,0);
	g(2,0) = (q.transpose()*Si*q_p3*exp_q)(0,0);
    ...
    H(0,0) = (-exp_q*( (-q.transpose()*Si*q_p1)*(-q.transpose()*Si*q_p1)+(-q_p1.transpose()*Si*q_p1)))(0,0);
    H(0,1) = (-exp_q*( (-q.transpose()*Si*q_p1)*(-q.transpose()*Si*q_p2)+(-q_p2.transpose()*Si*q_p1)))(0,0);
    H(0,2) = (-exp_q*( (-q.transpose()*Si*q_p1)*(-q.transpose()*Si*q_p3)+(-q_p3.transpose()*Si*q_p1)))(0,0);
    H(1,0) = (-exp_q*( (-q.transpose()*Si*q_p2)*(-q.transpose()*Si*q_p1)+(-q_p1.transpose()*Si*q_p2)))(0,0);
    H(1,1) = (-exp_q*( (-q.transpose()*Si*q_p2)*(-q.transpose()*Si*q_p2)+(-q_p2.transpose()*Si*q_p2)))(0,0);
    H(1,2) = (-exp_q*( (-q.transpose()*Si*q_p2)*(-q.transpose()*Si*q_p3)+(-q_p3.transpose()*Si*q_p2)))(0,0);
    H(2,0) = (-exp_q*( (-q.transpose()*Si*q_p3)*(-q.transpose()*Si*q_p1)+(-q_p1.transpose()*Si*q_p3)))(0,0);
    H(2,1) = (-exp_q*( (-q.transpose()*Si*q_p3)*(-q.transpose()*Si*q_p2)+(-q_p2.transpose()*Si*q_p3)))(0,0);
    H(2,2) = (-exp_q*( (-q.transpose()*Si*q_p3)*(-q.transpose()*Si*q_p3)+(-q.transpose()*Si*q_pp)+(-q_p3.transpose()*Si*q_p3)))(0,0);
	H_previous += H;
	g_previous += g;
