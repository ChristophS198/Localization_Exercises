...
int main() {  
  ...
  for (int t = 0; t < time_steps; ++t) {
    ...
    for (unsigned int i = 0; i < map_size; ++i) {
      ...
      // accumulated probability of ending at current pseudo_position when starting
      // anywhere on the map and moving movement_per_timestep + variance control_stdev
      float motion_prob = motion_model(pseudo_position, movement_per_timestep,
                                        priors, map_size, control_stdev);

      // get pseudo ranges - landmark_positions as seen from current pseudo_position
      vector<float> pseudo_ranges = pseudo_range_estimator(landmark_positions, 
                                                            pseudo_position);


      // get observation probability
      // compares pseudo_ranges (what we expect to see at current pseudo_position)
      // with observations (what we really see) given observation_stdev
      float obs_prob = observation_model(landmark_positions, 
                                          observations, pseudo_ranges,
                                          distance_max, observation_stdev);


      // calculate the ith posterior and pass to posteriors vector
      posteriors[i] = obs_prob * motion_prob;
