...
// The notation for source is the input scan and target is what you are trying to align the scan to, 
// which in this case is the point cloud map
...
int main(){
	...
	//TODO: Set resolution and point cloud target (map) for ndt
  	// Setting point cloud to be aligned to. mapCloud is only set once so 
	// PDFs are only calculated once and not for each iteration
